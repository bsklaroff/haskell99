...
main = do putStrLn . show $ connectedcomponents (Graph [1,2,3,4,5,6,7] [(1,2),(2,3),(1,4),(3,4),(5,2),(5,4),(6,7)])
...
connectedcomponents :: (Eq a, Ord a) => G a -> [[a]]
connectedcomponents (Graph gns ges) = cc (Graph gns ges) gns
  where cc _ [] = []
        cc g (n:ns) = nextClump : cc g (filter (`notElem` nextClump) ns)
          where nextClump = depthfirst g n
